...
class newProfilometer():
    ...
    DELTA_T = 0.028  #delay between trigger and pose
    #Session parameters
    SC = 0
    ...
    R_EZ = math.radians(0.80838)#(0.67525)
    R_EY = math.radians(-0.104061)#(-0.10451)
    R_EX = math.radians(180.58144)#(179.8448)
    ProfilometerCoordSystem = FrankUtilities.Euler2RotMatrix([R_EZ, R_EY, R_EX])
    ProfilometerOffset =[48.9143, -15.068, -71.7197]#[48.924, -14.8584, -72.91437]# 205.06482242638083]#50.56595726497499]# [1.919597951,49.27719208,52.3350607]#[1.502,48.916,50.774]#[1.0717818961710721,48.072654304144066,51.03132584386907] #[1.0714803654320781, 48.64106065763202, 50.56595726497499]  #Offset of the profilmeter with respect to tool origin #Check Sign
    ...
    def Acquire_trigger(self,trig_starter, SavePath='', SaveToFile=False, RobotLinSpeed = 100, xScale = 0.125, ProfilesPerFrame =100):    ###### xScale=0.2 ######## profilesperframe = 100
        ...
        while True:
            ...
            Profile=[]
            # Profile.append(temp)
            # Profile =list(np.squeeze(np.asarray(Profile)))
        ...
        #forse thread questa parte

        if SaveToFile:
            #SAVE PROFILOMETER DATA
            print("Profilometer acquisition: saving to file")

            #file = open(SavePath+"/Profiles_data"+str(self.id_acqu)+'.txt',"r")
            # line = file.readline()
            # file.close()
            # divisi = line.split("\t")
        ...
        t2 = threading.Thread(target =self.SyncronizePointclouds_threader, args = (SavePath,self.id_acqu,self.DELTA_T))
        t2.start() # start SINCRONIZATION OF THE PROFILES AND ROBOT POSES
        return #[XX,ZZ]# [XX, YY, ZZ]

    def SyncronizePointclouds_threader(self,filepath,num,DELTA_T):
        syncronizer.sync_function(filepath,num,DELTA_T)
