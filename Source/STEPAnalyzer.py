...
from OCC.Core.TopoDS import TopoDS_Shape, topods, topods_Face, topods_Vertex, topods_Edge, TopoDS_Compound, topods_Shell
...
class StepAnalyzer():
    ...
    def find_axis_color(self,filename, color_list):
        ...
        for i in range(len(colors)):
            ...
            if c == [0,0,1]:
               ...
               bol_b.append(0)
        shape_ls_r = [shape_ls[i] for i in range(len(shape_ls)) if bol_r[i]] # extract all of the shape with the specified color
        shape_ls_g = [shape_ls[i] for i in range(len(shape_ls)) if bol_g[i]] # extract all of the shape with the specified color
        shape_ls_b = [shape_ls[i] for i in range(len(shape_ls)) if bol_b[i]] # extract all of the shape with the specified color
        ...
        shape_g = BuildCompoundFromShapes(shape_ls_g)
        shape_r = BuildCompoundFromShapes(shape_ls_r)
        shape_b = BuildCompoundFromShapes(shape_ls_b)
        shapes = [shape_r,shape_g,shape_b]
        coords_frame = []
        for s in shapes:
            edges_ls_g = get_edges(s)
            print(edges_ls_g)
            first_pts_ls = []
            last_pts_ls = []
            count = 0
            for e in edges_ls_g:   
               #
                ...
                if not first.IsNull(): # remove closed edge
                    ...
                    print(count)
                    count+=1
            first_pts_ls = np.squeeze(np.array(first_pts_ls))
            last_pts_ls = np.squeeze(np.array(last_pts_ls))
       ...
       # last_pc.paint_uniform_color([1, 0, 0])
            ...
            job = np.asarray([first_pts_ls, last_pts_ls])
            print(job)
            coords_frame.append(job)
        coords_frame = np.concatenate(np.asarray(coords_frame),axis = 0)
        print(coords_frame)
        return  coords_frame # , line_set
    ...
    def Cluster_and_OrderPoints(self,m_ls,last_pts_ls,first_pts_ls,forProfi=True,newAlg=True):
        '''
        ...
        if newAlg ==True:
            cluster_mid, cluster_last, cluster_first, leftovers  = self.Clustering_Points_new(m_ls,last_pts_ls,first_pts_ls)   #, leftovers
        else:
       ...
       # o3d.visualization.draw_geometries([NumpyToPCD(leftovers)])
        ...
        for i in range(len(cluster_mid_profi)):
            ...
            dist = 50
            out = self.add_tails_to_cluster(cluster_mid_profi[i], dist)
            cluster_mid_profi = list(cluster_mid_profi)
            cluster_mid_profi[i] = []
            cluster_mid_profi[i] = out

            #first
            out = self.add_tails_to_cluster(cluster_first_profi[i], dist)
            cluster_first_profi = list(cluster_first_profi)
            cluster_first_profi[i] = []
            cluster_first_profi[i] = out
            #last
            out = self.add_tails_to_cluster(cluster_last_profi[i], dist)
            cluster_last_profi= list(cluster_last_profi)
            cluster_last_profi[i] = []
            cluster_last_profi[i] = out

            cluster_mid_profi[i] = np.squeeze(cluster_mid_profi[i])
            cluster_first_profi[i] = np.squeeze(cluster_first_profi[i])
            cluster_last_profi[i] = np.squeeze(cluster_last_profi[i])
                ## print("ok")
        for i in range(len(cluster_mid_profi)):
        ...
        print("ok")
        return [cluster_mid, cluster_last, cluster_first, vectors, clusters_labels], [cluster_mid_profi,cluster_first_profi,cluster_last_profi, vectors, clusters_labels_profi]
    ...
    def TCP_pose_from_axis_mockup(self, cluster_mid, cluster_last, cluster_first, vectors,clusters_labels, filepath='', forProfi = False, SaveFile = True ,offset = 0):
        ...
        for i in range(len(cluster_mid)):
            Quat,RT,Eul = rotation_from_3points(cluster_first[i],cluster_last[i],vectors[i])
            RT = np.asarray(RT)
        ...
        v3= v3/norm
        if forProfi:
            off_pts = offset_along_vector(m_ls, -v1, offset)
        else:
            off_pts =m_ls
        VISUALIZE = True  
        draw = []    
        draw.append(NumpyToPCD(off_pts).paint_uniform_color([0,0,0]))                      # allow visualization of the TCP reference frame to check if the computed orientation are correct
        for distance in range(1,10,1):
            px = offset_along_vector(off_pts[:],v3[:],distance)
            draw.append(NumpyToPCD(px).paint_uniform_color([1,0,0]))
        for distance in range(1,10,1):
            py = offset_along_vector(off_pts[:],v2[:],distance)
            draw.append(NumpyToPCD(py).paint_uniform_color([0,1,0]))
        for distance in range(1,10,1):
            pz = offset_along_vector(off_pts[:],v1[:],distance)
    ...
    def add_tails_to_cluster(self,cluster, dist):
        '''
    ...
    def Clustering_Points_new(self, points,normal,antinormal, min_ratio = 0.02):   #min_ratio  = %of leftovers accepted
        N = len(points)
        ...
        while count < (1 - min_ratio) * N:
            model_robust, inliers = ransac(target, LineModelND, min_samples=2,
                                residual_threshold=math.ceil(len(target)/15), max_trials=5000)    #15 5000# residual_threshold and max_trials main parameters to tune for desired clusterization outcome
        ...
        return center_list, normal_list, antinormal_list, target
